...
const project_model_1 = __importDefault(require("./modules/project/project.model"));
...
const iconUrl = (name) => `https://api.iconify.design/lucide:${name}.svg?color=%234f46e5`;
const imageUrl = (seed) => `https://picsum.photos/seed/${seed}/800/600`;
const commonFields = {
    idealFor: [
...
async function seedDatabase() {
    try {
        ...
        let admin = await admin_model_1.default.findOne();
        if (!admin) {
            console.log('No admin found, creating a dummy admin');
            admin = await admin_model_1.default.create({
                name: 'Seed Admin',
        ...
        await project_model_1.default.insertMany(projectsWithCreator);
